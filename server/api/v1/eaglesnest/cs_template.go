...
func (t *TemplateApi) MigrateTable(c *gin.Context) {
	err := global.GVA_DB.AutoMigrate(&eaglesnest.Template{})
	if err != nil {
		response.FailWithMessage(err.Error(), c)
		return
	}
	err = global.GVA_DB.AutoMigrate(&eaglesnest.Area{})
	err = global.GVA_DB.AutoMigrate(&eaglesnest.JobResultItem{})
	response.Ok(c)
}
...
func (t *TemplateApi) CreateTemplate(c *gin.Context) {
	...
	var modelTemplate = eaglesnest.Template{
...
func (t *TemplateApi) ImportTemplates(c *gin.Context) {
	...
	templates := make([]*eaglesnest.Template, 0)
