...
import (
	...
	request2 "github.com/EscapeBearSecond/eaglesnest/server/model/eaglesnest/request"
...
func (t *TemplateService) CreateTemplate(template *eaglesnest.Template) error {
	if !errors.Is(global.GVA_DB.Select("template_name").First(&eaglesnest.Template{}, "template_name = ?", template.TemplateName).Error, gorm.ErrRecordNotFound) {
...
func (t *TemplateService) DeleteTemplate(id int) error {
	return global.GVA_DB.Delete(&eaglesnest.Template{}, id).Error
}

// GetTemplateById 根据模板ID获取模板详情信息，包括模板内容
func (t *TemplateService) GetTemplateById(id int) (*eaglesnest.Template, error) {
	var template eaglesnest.Template
...
func (t *TemplateService) GetTemplatesByIds(ids []int64) ([]*eaglesnest.Template, error) {
	var templates []*eaglesnest.Template
	err := global.GVA_DB.Select("id", "template_name", "template_type", "template_id", "template_desc", "template_content",
...
func (t *TemplateService) GetTemplateList(searchTemplate request2.SearchTemplate) (list []*eaglesnest.Template, total int64, err error) {
	...
	if searchTemplate.IsAll {
		db = global.GVA_DB.Model(&eaglesnest.Template{})
	} else {
		db = global.GVA_DB.Model(&eaglesnest.Template{}).Omit("template_content")
	}
	var templates []*eaglesnest.Template
...
func (t *TemplateService) UpdateTemplate(template *eaglesnest.Template) error {
	var existingRecord eaglesnest.Template
...
func (t *TemplateService) BatchAdd(templates []*eaglesnest.Template) error {
	// uniqueTemplates := make(map[string]*eaglesnest.Template)
	...
	// }
	//
	// var deduplicatedTemplates []*eaglesnest.Template
...
func (t *TemplateService) ParseTemplateContent(template *eaglesnest.Template) (err error) {
