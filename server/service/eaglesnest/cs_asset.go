...
func (a *AssetService) BatchAdd(assets []*eaglesnest.Asset) error {
...
func (a *AssetService) BatchAddWithTransaction(tx *gorm.DB, assets []*eaglesnest.Asset) error {
	// seen := make(map[string]struct{})
	// var result []*eaglesnest.Asset
...
func (a *AssetService) GetAssetList(asset *eaglesnest.Asset, page request.PageInfo, order string, desc bool, allData bool) (list interface{}, total int64, err error) {
	limit := page.PageSize
	offset := page.PageSize * (page.Page - 1)
	db := global.GVA_DB.Select("id", "asset_name", "asset_ip", "asset_area", "area_name", "asset_type", "open_ports", "system_type",
		"ttl", "asset_model", "manufacturer", "created_at", "updated_at", "deleted_at").Model(&eaglesnest.Asset{})
