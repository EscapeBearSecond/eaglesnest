...
func (p *PolicyService) CreatePolicy(policy *eaglesnest.Policy) error {
	if !errors.Is(global.GVA_DB.Select("policy_name").First(&eaglesnest.Policy{}, "policy_name = ?", policy.PolicyName).Error, gorm.ErrRecordNotFound) {
...
func (p *PolicyService) UpdatePolicy(policy *eaglesnest.Policy) error {
	var existingRecord eaglesnest.Policy
...
func (p *PolicyService) DeletePolicy(id int) error {
	...
	return global.GVA_DB.Delete(&eaglesnest.Policy{}, id).Error
}
...
// 返回值:
//
//	*eaglesnest.Policy - 找到的策略对象，如果未找到则为nil。
//	error - 查询过程中发生的错误，如果未找到记录，则错误消息为“目标数据不存在”。
func (p *PolicyService) GetPolicyById(id int) (*eaglesnest.Policy, error) {
	var policy eaglesnest.Policy
...
// 返回值:
//
//	list: 策略列表，类型为interface{}，具体为[]eaglesnest.Policy。
//	total: 策略总条数。
//	err: 查询过程中可能出现的错误。
func (p *PolicyService) GetPolicyList(policy eaglesnest.Policy, page request.PageInfo, order string, desc bool) (list interface{}, total int64, err error) {
	limit := page.PageSize
	offset := page.PageSize * (page.Page - 1)
	db := global.GVA_DB.Model(&eaglesnest.Policy{}).Omit("policy_config", "online_config", "port_scan_config")
	var policies []eaglesnest.Policy
